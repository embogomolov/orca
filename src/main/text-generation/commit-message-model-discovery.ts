...
import { getSpawnArgsForWindows, WINDOWS_BATCH_UNSAFE_ARGUMENTS_ERROR } from '../win32-utils'
import {
...
function startCodexAppServerModelDiscovery(input: {
...
export async function discoverModelsLocal(input: {
  ...
  if (input.agentId === 'codex' && input.options.includeSessionDefaults && !input.options.wslDistro) {
