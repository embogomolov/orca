...
export async function runRoomSteer(
  ...
  adapters: Record<string, RoomHarnessAdapter>,
  id: string,
  requestFence: (roomId: string, options: { discardConfirmations: boolean }) => RoomDeliveryFence,
  ...
  if (
    targetIds.some(
      (participantId) => db.messages.deliveries.delivering(participantId)?.intent === 'steer'
    )
  ) {
    throw new Error('conversation_steer_busy')
  }
  const fence = requestFence(roomId, { discardConfirmations: false })
  try {
    await fence.ready
